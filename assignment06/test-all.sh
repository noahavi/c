...
TEST_CASE_RANGE=6
NUM_TEST_CASES=18
TOTAL_TEST_CNT=19  # Three tests per function, plus the compiler warning check.
...
if [ ! -f assignment06.c ]; then
	echo ""
	echo " Error: You must place assignment06.c in this directory before we can"
	echo "        proceed. Aborting test script."
	echo ""
	exit
elif [ ! -f assignment06.h ]; then
	echo ""
	echo " Error: You must place assignment06.h in this directory before we can"
	echo "        proceed. Aborting test script."
	echo ""
	exit
elif [ ! -f pancake_stuff.h ]; then
	echo ""
	echo " Error: You must place pancake_stuff.h in this directory before we can"
	echo "        proceed. Aborting test script."
	echo ""
	exit
...
for i in `seq -f "%02g" 1 $TEST_CASE_RANGE`;
do
	check_test_case "$i"a
	check_test_case "$i"b
	check_test_case "$i"c
...
egrep -s -q "^#include \"assignment06\.h\"" assignment06.c
egrep_val=$?

if [[ $egrep_val != 0 ]]; then
	echo ""
	echo "  Whoa, buddy! Your assignment06.c file does not appear to have a proper"
	echo "  #include statement for assignment06.h. Please read Section 1 of the"
...
run_test_case()
{
	...
	# Attempt to compile.
	gcc assignment06.c $testcase_file 2> /dev/null
	compile_val=$?
...
echo ""
echo "  Total tests passed: $PASS_CNT/$NUM_TEST_CASES"
